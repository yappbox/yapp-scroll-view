...
function captureClick(e) {
  e.stopPropagation();
  e.target.closest('.ScrollView').removeEventListener('click', captureClick, true);
}
class ScrollView extends Component {
  ...
  static {
    g(this.prototype, "scrollBarClientHeight", [tracked]);
  }
  #scrollBarClientHeight = (i(this, "scrollBarClientHeight"), void 0);
  static {
    g(this.prototype, "scrollBarContentHeight", [tracked]);
  }
  #scrollBarContentHeight = (i(this, "scrollBarContentHeight"), void 0);
  _appliedScrollTop;
  ...
  static {
    g(this.prototype, "memory", [inject('scroll-position-memory')]);
  }
  ...
  static {
    n(this.prototype, "onContentHeightChanged", [action]);
  }
  didInsert(element) {
  ...
  static {
    n(this.prototype, "onKeyUpdated", [action]);
  }
  willDestroy() {
  ...
  applyScrollTop({
    ...
    let {
      isAtTop,
      isAtTopChanged
    } = getScrolledToTopChanged(scrollTop, lastTop, this.scrollTopOffset);
  ...
  bindScrollerEvents(element) {
    this.hammer = new Hammer.Manager(element, {
      ...
      recognizers: [[ZyngaScrollerVerticalRecognizer, {
        scrollComponent: this
      }]]
    });
    this._boundHandleWheel = this.handleWheel.bind(this);
    element.addEventListener(normalizeWheel.getEventType(), this._boundHandleWheel, {
      passive: false
    });
    ...
    this._scrollViewDocument.addEventListener('click', this._boundHandleCapturedClick, true);
  }
  unbindScrollerEvents(element) {
    ...
    element?.removeEventListener(normalizeWheel.getEventType(), this._boundHandleWheel, false);
    this._scrollViewDocument?.removeEventListener('click', this._boundHandleCapturedClick, true);
  ...
  doTouchStart(touches, timeStamp) {
    ...
    this._touchStartWasDecelerating = !!(scroller?.__isDecelerating || scroller?.__isAnimating);
    this._touchStartDecelerationVelocityY = scroller?.__decelerationVelocityY ?? this._decelerationVelocityY ?? 0;
    this._wasScrollingAtTouchStart = this._isScrolling || this._touchStartWasDragging || this._touchStartWasDecelerating;
  ...
  needsPreventClick(touchDuration) {
    let momentumVelocity = this._touchStartDecelerationVelocityY ?? this._decelerationVelocityY ?? 0;
    let isFinishingDragging = this._touchStartWasDragging || this.scroller.__isDragging;
    let wasAnimatingWithMomentum = this._touchStartWasDecelerating || this._wasScrollingAtTouchStart && Math.abs(momentumVelocity) > 0.5;
  ...
  handleWheel(e) {
    ...
    e.preventDefault();
    if (scrollTop === 0 && candidatePosition < 0 || scrollTop === maxScrollTop && candidatePosition > maxScrollTop) {
      return;
    }
  ...
  updateScrollBarDimensionsTask = buildTask(() => ({
    context: this,
    generator: function* () {
  ...
  }), null, "updateScrollBarDimensionsTask", null);
  get extraCssClasses() {
    return null;
  }
  get windowRef() {
  ...
  static {
    n(this.prototype, "scrollViewApi", [cached]);
  }
...
var ScrollView$1 = setComponentTemplate(TEMPLATE, ScrollView);
